...
class PriorityQueue:
    ...
    def push(self, item, priority, path):
        heapq.heappush(self.__queue__,(priority, item[0], item[1], path))

    def pop(self):
        (priority, pac_pos, food_grid, path) = heapq.heappop(self.__queue__)
        return (priority, (pac_pos, food_grid), path)

    def isEmpty(self):
        return not self.__queue__

    def update(self, item, priority, path):
        flag = 0
        for (pri, pac_pos, food_grid, p) in self.__queue__:
            if pac_pos == item[0]:
                flag = 1
                if priority > pri:
                    return
                else:
                    self.__queue__.remove((pri, pac_pos, food_grid, p))
                    self.push(item, priority, path)
                    return
        if flag == 0:
            self.push(item, priority, path)
...
class PriorityQueueWithFunction(PriorityQueue):
    ...
    def push(self, item, path):
        PriorityQueue.push(self, item, self.function(item, self.problem), path)
    ...
    def update(self, item, path):
        ...
        for (pri, pac_pos, food_grid, p) in self.__queue__:
            if pac_pos == item[0]:
                flag = 1
                if self.function(item, self.problem) > pri: # If item already in priority queue with equal or lower priority, do nothing.
                    return
                else:  # If item already in priority queue with higher priority, update its priority and rebuild the heap.
                    self.__queue__.remove((pri, pac_pos, food_grid, p))
                    self.push(item, path)
                    return
        if flag == 0: # If item not in priority queue, do the same thing as self.push.
            self.push(item, path)
